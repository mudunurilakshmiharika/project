const API_URL = window.location.hostname === "localhost" || window.location.hostname === "127.0.0.1"
    ...
    : "https://project-12-d2tc.onrender.com/api/products";
...
const FALLBACK_PRODUCTS = [
  ...
  { _id: "f6", name: "Chino Trousers", price: 2799, image: "https://images.unsplash.com/photo-1594633312681-425c7b97ccd1?w=800", category: "Men's Fashion", description: "Smart-casual beige chinos, catalog style." },
    ...
    if (container && allProducts.length === 0) {
      container.innerHTML = `
        <div style="grid-column: 1/-1; text-align: center; padding: 2rem; background: #fee2e2; border-radius: 1rem; color: #991b1b;">
          ...
          <p style="font-size: 0.875rem; margin-top: 0.5rem;">Make sure your backend is running at https://project-12-d2tc.onrender.com</p>
        </div>
      `;
    }
  }
...
async function addToCart(id, showAlert = true) {
  if (!checkLogin()) return;
...
async function buyNow(id) {
  const success = await addToCart(id, false);
  if (success) {
